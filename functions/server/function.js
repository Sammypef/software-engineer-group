...
exports.Register = (pool) => async (req, res) => {
    //Get info from frontend
    const {email, password} = req.body;
    // console.log('Register endpoint called for email:', email); // Removed for production logging
    try {
      if (!email || !password) {
...
exports.Validate_mail = (pool) => async(req, res) => { // Don't use this function for now
    //Get info from frontend 
    const {code} = req.body;
...
exports.Song_Play = (pool) => async (req, res) => {
  try {
    const { id } = req.params;
